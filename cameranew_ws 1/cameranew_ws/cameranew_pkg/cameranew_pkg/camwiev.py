...
# =================================================
# KAMERA TANIMLARI
# 4 webcam + 1 ZED kamera. Tuslar:
#   1 -> web1, 2 -> web2, 3 -> web3, 4 -> web4, 5 -> ZED
# camera_manager.py ile ayni isimler kullanilmali.
# =================================================
CAMERAS = ["web1", "web2", "web3", "web4", "ZED"]
...
class RoverCameraViewer(Node):
    def __init__(self):
        ...
        # ornegin ZED kamera yogunken diger kameralarin ve
        ...
        # ARAYÜZ
        # =================================================
        self.win_name = "Yildiz Rover Camera Viewer - 4 WEB + 1 IP"
        ...
        self.get_logger().info("5 -> ZED CAM ac/kapat")
        self.get_logger().info("q -> Hepsini kapat ve cik")

        for name in CAMERAS:
            self.get_logger().info(f"{name} topic: {CAMERA_TOPICS[name]}")

        # Arayüz yenileme. 15 Hz -> 30 Hz: cizim daha sik yenilenir,
        # kare bir sonraki cizime kadar en fazla ~33 ms bekler.
        # NOT: Timer kaldirildi, cunku OpenCV GUI fonksiyonlari (imshow, waitKey)
        # ana thread'de calismalidir. Timer arkaplan thread'inde calistiginda
        # pencere odaktayken (focus) X11 deadlock olup donmalara sebep oluyordu.
        # process_loop artik main() icinde ana thread'den cagiriliyor.
    # CAMERA MANAGER STATUS
    # =================================================
    def camera_status_cb(self, msg):
        """
        Beklenen format:
        web1:ON,web2:OFF,web3:ON,web4:OFF,ZED:OFF
        """
    ...
    def process_loop(self):
        try:
            ...
            # X11 Key Repeat Buffer Flush (Pencere odakliyken basili tutulan tuslarin
            # arayuzu dondurmesini engellemek icin kuyruktaki tum tuslari oku ve erit)
            # NOT: Sonsuz dongu (while True) yerine maksimum 15 kez okuma yapiyoruz ki
            # basili tutma durumunda dongu icinde sonsuza kadar hapis kalip arayuz donmasin.
            keys_pressed = set()
            for _ in range(15):
                k = cv2.waitKey(1)
                if k == -1:
...
def main(args=None):
    ...
    # ROS 2 dinleyicilerini arka planda çalıştır (OpenCV'yi engellememek için)
    spin_thread = threading.Thread(target=executor.spin, daemon=True)
    spin_thread.start()

    try:
        # OpenCV arayüz döngüsü ZORUNLU OLARAK ana thread'de çalışmalıdır.
        # (Aksi takdirde pencere odaklandığında klavye okurken X11/Linux arayüzü kilitlenir)
